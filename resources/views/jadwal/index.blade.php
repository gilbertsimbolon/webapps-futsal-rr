...
<div class="modal fade" id="modalGenerateSlot" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <form class="modal-content" action="{{ route('jadwal.generate') }}" method="POST">
            @csrf
            <div class="modal-header">
            ...
            <div class="modal-body">
                <p class="text-muted small mb-3">Sistem akan secara otomatis memecah jam buka s/d jam tutup menjadi slot berdurasi 1 jam.</p>
                <div class="mb-3">
                ...
                <div class="row g-2 mb-3">
                    <div class="col-6">
                        ...
                        <input type="text" name="open_time" class="form-control" placeholder="08:00" value="08:00" required>
                    </div>
                    <div class="col-6">
                        <label class="form-label">Jam Tutup <span class="text-danger">*</span></label>
                        <input type="text" name="close_time" class="form-control" placeholder="23:00" value="23:00" required>
                    </div>
                </div>
            </div>
            <div class="modal-footer">
...
@foreach ($schedules as $schedule)
    ...
    <!-- Modal Edit Slot -->
    <div class="modal fade" id="modalEditSlot{{ $schedule->id }}" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered">
            <form class="modal-content" action="{{ route('jadwal.update', $schedule->id) }}" method="POST">
                @csrf
                @method('PUT')
                <div class="modal-header">
                    <h5 class="modal-title">Edit Slot: {{ $startTimeVal }} - {{ $endTimeVal }}</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    <div class="mb-3">
                        ...
                        <select name="field_id" class="form-select" required>
                            @foreach ($fields as $field)
                                <option value="{{ $field->id }}" {{ old('field_id', $schedule->field_id) == $field->id ? 'selected' : '' }}>
                                    {{ $field->branch?->branch_name }} - {{ $field->field_name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="mb-3">
                        <label class="form-label">Hari Berlaku <span class="text-danger">*</span></label>
                        <select name="day" class="form-select" required>
                            <option value="all" {{ old('day', $schedule->day) == 'all' ? 'selected' : '' }}>Setiap Hari (Senin - Minggu)</option>
                    ...
                    <div class="row g-2 mb-3">
                        <div class="col-6">
                            <label class="form-label">Jam Mulai <span class="text-danger">*</span></label>
                            <input type="text" name="start_time" class="form-control" value="{{ old('start_time', $startTimeVal) }}" required>
                        </div>
                        <div class="col-6">
                            <label class="form-label">Jam Selesai <span class="text-danger">*</span></label>
                            <input type="text" name="end_time" class="form-control" value="{{ old('end_time', $endTimeVal) }}" required>
                        </div>
                    </div>
                    <div class="mb-3">
                        <label class="form-label">Harga Khusus Slot (Opsional)</label>
                        <input type="number" name="custom_price" class="form-control" value="{{ old('custom_price', $schedule->custom_price ? (int)$schedule->custom_price : '') }}" placeholder="Default harga lapangan" min="0">
                    </div>
                    <div class="mb-3">
                        <label class="form-label">Status Slot <span class="text-danger">*</span></label>
                        <select name="status" class="form-select" required>
                            <option value="active" {{ old('status', $schedule->status) == 'active' ? 'selected' : '' }}>Aktif (Bisa Disewa)</option>
