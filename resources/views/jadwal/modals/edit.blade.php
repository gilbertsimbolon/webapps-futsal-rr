<div class="modal fade" id="modalEditSlot{{ $schedule->id }}" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <form class="modal-content" action="{{ route('pemilik.jadwal.update', $schedule->id) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="modal-body">
                ...
                <div class="mb-3">
                    ...
                    <select name="day" class="form-select" required>
                        <option value="all" {{ old('day', $schedule->day) == 'all' ? 'selected' : '' }}>Setiap Hari
                            (Senin - Minggu)</option>
                        ...
                        <option value="sunday" {{ old('day', $schedule->day) == 'sunday' ? 'selected' : '' }}>Minggu
                        </option>
                    </select>
                </div>
                <div class="row g-2 mb-3">
                    <div class="col-6">
                        ...
                        <input type="text" name="start_time" class="form-control"
                            value="{{ old('start_time', $startTimeVal) }}" required>
                    </div>
                    <div class="col-6">
                        <label class="form-label">Jam Selesai <span class="text-danger">*</span></label>
                        <input type="text" name="end_time" class="form-control"
                            value="{{ old('end_time', $endTimeVal) }}" required>
                    </div>
                </div>
                <div class="mb-3">
                    <label class="form-label">Harga Khusus Slot (Opsional)</label>
                    <input type="number" name="custom_price" class="form-control"
                        value="{{ old('custom_price', $schedule->custom_price ? (int) $schedule->custom_price : '') }}"
                        placeholder="Default harga lapangan" min="0">
                </div>
                <div class="mb-3">
                    ...
                    <select name="status" class="form-select" required>
                        <option value="active" {{ old('status', $schedule->status) == 'active' ? 'selected' : '' }}>
                            Aktif (Bisa Disewa)</option>
                        <option value="inactive"
                            {{ old('status', $schedule->status) == 'inactive' ? 'selected' : '' }}>Nonaktif</option>
                    </select>
                </div>
            </div>
